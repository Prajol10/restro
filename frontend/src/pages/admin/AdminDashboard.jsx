...
const AdminDashboard = () => {
  ...
  const S = {
    ...
    msg: (success) => ({ 
      ...
      padding: '14px 24px', borderRadius: '8px', fontWeight: 600,
      backgroundColor: success ? 'rgba(74,222,128,0.15)' : 'rgba(239,68,68,0.15)',
      color: success ? '#4ade80' : '#f87171', border: '1px solid ' + (success ? 'rgba(74,222,128,0.3)' : 'rgba(239,68,68,0.3)')
    }),
    ...
    tab: (active, accent) => ({ 
      padding: '8px 16px', backgroundColor: active ? accent : 'transparent', 
      color: active ? '#000' : 'rgba(255,255,255,0.5)', border: 'none', 
  ...
  const archiveGalleryImage = async (id) => {
    await authFetch(`${API}/Admin/gallery/${id}/archive`, { method:'PUT' });
    fetchGallery(); showMsg('✅ Archived!');
  };

  const archiveMenuItem = async (id) => {
    await authFetch(`${API}/Admin/menu-items/${id}/archive`, { method:'PUT' });
    fetchMenuItems(); showMsg('✅ Archived!');
  };

  const restoreGalleryImage = async (id) => {
    await authFetch(`${API}/Admin/gallery/${id}/archive`, { method:'PUT' });
    fetchArchivedGallery(); showMsg('✅ Restored!');
  };

  const restoreMenuItem = async (id) => {
    await authFetch(`${API}/Admin/menu-items/${id}/archive`, { method:'PUT' });
    fetchArchivedItems(); showMsg('✅ Restored!');
  };
  ...
  const deleteCategory = async (id) => {
    ...
    await authFetch(`${API}/Admin/menu-categories/${id}`, { method:'DELETE' });
    fetchCategories(); showMsg('✅ Deleted!');
  };
  ...
  const saveItem = async (e) => {
    ...
    await authFetch(url, { method, body: JSON.stringify({ ...itemForm, categoryId:parseInt(itemForm.categoryId), price:parseFloat(itemForm.price) }) });
  ...
  const deleteItem = async (id) => {
    ...
    await authFetch(`${API}/Admin/menu-items/${id}`, { method:'DELETE' });
    fetchMenuItems(); showMsg('✅ Deleted!');
  };

  const updateOrderStatus = async (id, status) => {
    await authFetch(`${API}/Admin/orders/${id}/status`, { method:'PUT', body:JSON.stringify({ status }) });
    fetchOrders();
  };

  const updateReservationStatus = async (id, status) => {
    await authFetch(`${API}/Admin/reservations/${id}/status`, { method:'PUT', body:JSON.stringify({ status }) });
    fetchReservations();
  };
  ...
  const toggleApprove = async (id) => {
    await authFetch(`${API}/Admin/reviews/${id}/approve`, { method:'PUT' });
    fetchReviews();
  };
  ...
  const addGalleryImage = async (url) => {
    await authFetch(`${API}/Admin/gallery`, { method:'POST', body:JSON.stringify({ imageUrl:url, caption:'', sortOrder:0 }) });
    fetchGallery(); showMsg('✅ Image added!');
  };
  ...
  const saveWhyChooseUs = async (e) => {
    ...
    const method = editingWhyChooseUs ? 'PUT' : 'POST';
    await authFetch(url, { method, body: JSON.stringify(whyChooseUsForm) });
  ...
  const deleteWhyChooseUs = async (id) => {
    if (!confirm('Delete item?')) return;
    await authFetch(`${API}/Admin/why-choose-us/${id}`, { method:'DELETE' });
    fetchWhyChooseUs(); showMsg('✅ Deleted!');
  };

  const archiveWhyChooseUs = async (id) => {
    await authFetch(`${API}/Admin/why-choose-us/${id}/archive`, { method:'PUT' });
    fetchWhyChooseUs(); showMsg('✅ Archived!');
  };
  ...
  // Filter states for menu table
  const [menuSearch, setMenuSearch] = useState('');
  const [menuCategoryFilter, setMenuCategoryFilter] = useState('all');
  
  // Filter states for orders table
  const [orderStatusFilter, setOrderStatusFilter] = useState('all');
  
  // Filter states for reservations table
  ...
  return (
    <div style={S.page}>
      ...
      <div style={S.sidebar}>
        ...
        <div style={S.nav}>
          {tabs.map(tab => (
            <button key={tab.id} onClick={() => setActiveTab(tab.id)} style={S.navBtn(activeTab===tab.id, accent)}>
              <span>{tab.label}</span>
            </button>
          ))}
        </div>
        ...
        {activeTab==='dashboard' && (
          <div>
            ...
            <div style={S.statsGrid}>
              ...
              <div style={S.statCard}>
                <div style={S.statValue}>{stats.pendingOrders}</div>
                <div style={S.statLabel}>Pending Orders</div>
              </div>
              <div style={S.statCard}>
                <div style={S.statValue}>{stats.todayReservations}</div>
                <div style={S.statLabel}>Today's Reservations</div>
              </div>
              <div style={S.statCard}>
                <div style={S.statValue}>{stats.totalReviews}</div>
              ...
              <div style={S.tableContainer}>
                <table style={S.table}>
                  ...
                  <tbody>
                        ...
                        <td style={S.td}>
                          {(() => {
                            try {
                              ...
                              return Array.isArray(items) ? `${items.length} items` : '1 item';
                            } catch {
                              return '1 item';
                            }
                          })()}
                        </td>
                        <td style={S.td}>${parseFloat(order.totalAmount).toFixed(2)}</td>
                        <td style={S.td}>{new Date(order.createdAt).toLocaleDateString()}</td>
                        <td style={S.td}>
                          <span style={S.badge(...(STATUS_BADGE[order.status]||['#fff','rgba(255,255,255,0.1)']))}>
                            {order.status}
                          </span>
                        </td>
                      </tr>
                    ...
                    {orders.length === 0 && (
                      <tr>
                        <td colSpan="7" style={{...S.td, textAlign:'center', color:'rgba(255,255,255,0.3)'}}>No orders yet</td>
                      </tr>
                    )}
                  </tbody>
                </table>
                  ...
                  <tbody>
                    {reservations.slice(0, 5).map(res => (
                      <tr key={res.id} style={S.tr} onMouseEnter={(e) => e.target.closest('tr').style.backgroundColor = S.trHover.backgroundColor} onMouseLeave={(e) => e.target.closest('tr').style.backgroundColor = 'transparent'}>
                        <td style={S.td}>#{res.id}</td>
                        <td style={S.td}>{res.customerName}</td>
                        <td style={S.td}>{res.customerPhone}</td>
                        <td style={S.td}>{res.date}</td>
                        <td style={S.td}>{res.time}</td>
                        <td style={S.td}>{res.partySize} people</td>
                        <td style={S.td}>
                          <span style={S.badge(...(STATUS_BADGE[res.status]||['#fff','rgba(255,255,255,0.1)']))}>
                            {res.status}
                          </span>
                        </td>
                      </tr>
                    ...
                    {reservations.length === 0 && (
                      <tr>
                        <td colSpan="7" style={{...S.td, textAlign:'center', color:'rgba(255,255,255,0.3)'}}>No reservations yet</td>
                      </tr>
                    )}
                  </tbody>
                </table>
        ...
        {activeTab==='orders' && (
          <div>
            ...
            <div style={S.tabs}>
              ...
              <button onClick={() => setOrderStatusFilter('Confirmed')} style={S.tab(orderStatusFilter === 'Confirmed', accent)}>Confirmed</button>
              <button onClick={() => setOrderStatusFilter('Completed')} style={S.tab(orderStatusFilter === 'Completed', accent)}>Completed</button>
              <button onClick={() => setOrderStatusFilter('Cancelled')} style={S.tab(orderStatusFilter === 'Cancelled', accent)}>Cancelled</button>
            </div>
              ...
              <table style={S.table}>
                ...
                <tbody>
                        ...
                        <div style={{ display:'flex', gap:'8px', flexWrap:'wrap' }}>
                          {['Pending','Confirmed','Completed','Cancelled'].map(s => (
                            <button 
                              ...
                              style={{ 
                                padding:'4px 8px', borderRadius:'4px', border:'none', cursor:'pointer', fontSize:'11px', fontWeight:600, 
                                backgroundColor: order.status===s ? accent : 'rgba(255,255,255,0.05)', 
                                color: order.status===s ? '#000' : 'rgba(255,255,255,0.5)', 
                                transition:'all 0.2s' 
                              }}
                            >
                              {s}
                  ...
                  {orders.filter(order => orderStatusFilter === 'all' || order.status === orderStatusFilter).length === 0 && (
                    <tr>
                      <td colSpan="8" style={{...S.td, textAlign:'center', color:'rgba(255,255,255,0.3)'}}>No orders found</td>
                    </tr>
                  )}
                </tbody>
              </table>
        ...
        {activeTab==='reservations' && (
          <div>
            ...
            <div style={S.tabs}>
              ...
              <button onClick={() => setReservationStatusFilter('Confirmed')} style={S.tab(reservationStatusFilter === 'Confirmed', accent)}>Confirmed</button>
              <button onClick={() => setReservationStatusFilter('Cancelled')} style={S.tab(reservationStatusFilter === 'Cancelled', accent)}>Cancelled</button>
            </div>
            ...
            <div style={S.tableContainer}>
              <table style={S.table}>
                ...
                <tbody>
                  {reservations.filter(res => reservationStatusFilter === 'all' || res.status === reservationStatusFilter).map(res => (
                    <tr key={res.id} style={S.tr} onMouseEnter={(e) => e.target.closest('tr').style.backgroundColor = S.trHover.backgroundColor} onMouseLeave={(e) => e.target.closest('tr').style.backgroundColor = 'transparent'}>
                      <td style={S.td}>#{res.id}</td>
                      <td style={S.td}>{res.customerName}</td>
                      <td style={S.td}>{res.customerPhone}</td>
                      <td style={S.td}>{res.customerEmail}</td>
                      <td style={S.td}>{res.date}</td>
                      <td style={S.td}>{res.time}</td>
                      <td style={S.td}>{res.partySize} people</td>
                      <td style={S.td}>
                        ...
                        <div style={{ display:'flex', gap:'8px' }}>
                          {['Pending','Confirmed','Cancelled'].map(s => (
                            <button 
                              ...
                              style={{ 
                                padding:'4px 8px', borderRadius:'4px', border:'none', cursor:'pointer', fontSize:'11px', fontWeight:600, 
                                backgroundColor: res.status===s ? accent : 'rgba(255,255,255,0.05)', 
                                color: res.status===s ? '#000' : 'rgba(255,255,255,0.5)' 
                              }}
                            >
                              {s}
                            </button>
                  ...
                  {reservations.filter(res => reservationStatusFilter === 'all' || res.status === reservationStatusFilter).length === 0 && (
                    <tr>
                      <td colSpan="9" style={{...S.td, textAlign:'center', color:'rgba(255,255,255,0.3)'}}>No reservations found</td>
                    </tr>
                  )}
                </tbody>
              </table>
        ...
        {activeTab==='menu' && (
          <div>
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'24px' }}>
              ...
              <div style={{ display:'flex', gap:'8px' }}>
                <button onClick={() => { setShowCatForm(!showCatForm); setEditingCat(null); setCatForm({ name:'', description:'', sortOrder:0 }); }} style={S.btnOutline}>
                  {showCatForm ? 'Cancel' : '+ Category'}
                </button>
                <button onClick={() => { setShowItemForm(!showItemForm); setEditingItem(null); setItemForm({ categoryId: categories[0]?.id||'', name:'', subtitle:'', description:'', price:'', imageUrl:'', isSpecial:false, isAvailable:true, sortOrder:0 }); }} style={S.btnPrimary(accent)}>
                  {showItemForm ? 'Cancel' : '+ Menu Item'}
                </button>
              </div>
            </div>
                ...
                <form onSubmit={saveCategory}>
                  <div style={S.grid2}>
                    <div>
                      ...
                      <input value={catForm.name} onChange={e => setCatForm({ ...catForm, name:e.target.value })} required style={S.input} />
                    </div>
                    <div>
                      <label style={S.label}>Description</label>
                      <input value={catForm.description} onChange={e => setCatForm({ ...catForm, description:e.target.value })} style={S.input} />
                    </div>
                  </div>
                ...
                <form onSubmit={saveItem}>
                  <div style={S.grid2}>
                    <div>
                      ...
                      <select value={itemForm.categoryId} onChange={e => setItemForm({ ...itemForm, categoryId:e.target.value })} required style={S.select}>
                    ...
                    <div>
                      <label style={S.label}>Name *</label>
                      <input value={itemForm.name} onChange={e => setItemForm({ ...itemForm, name:e.target.value })} required style={S.input} />
                    </div>
                    <div>
                      <label style={S.label}>Subtitle <span style={{ color:'rgba(255,255,255,0.25)', fontWeight:400, textTransform:'none', letterSpacing:0 }}>(e.g. LIMITED-TIME)</span></label>
                      <input value={itemForm.subtitle} onChange={e => setItemForm({ ...itemForm, subtitle:e.target.value })} style={S.input} />
                    </div>
                    <div>
                      <label style={S.label}>Price * ($)</label>
                      <input type="number" step="0.01" min="0" value={itemForm.price} onChange={e => setItemForm({ ...itemForm, price:e.target.value })} required style={S.input} placeholder="0.00" />
                    </div>
                    <div style={{ gridColumn:'1/-1' }}>
                      <label style={S.label}>Description</label>
                      <textarea value={itemForm.description} onChange={e => setItemForm({ ...itemForm, description:e.target.value })} rows={3} style={S.textarea} />
                    </div>
                    <div style={{ gridColumn:'1/-1' }}>
                      ...
                      <div style={{ display:'flex', gap:'8px' }}>
                        ...
                        <label style={S.uploadBtn}>
                          ...
                          <input type="file" accept="image/*" style={{ display:'none' }} onChange={async e => {
                            ...
                            try { const url = await uploadFile(file, slug, 'menu'); setItemForm(f => ({ ...f, imageUrl:url })); }
                            catch(err) { alert('Upload failed: '+err.message); }
                          }} />
                        </label>
                        <button type="button" onClick={() => setFilePicker({ onSelect:(url) => setItemForm(f => ({ ...f, imageUrl:url })) })} style={{ ...S.uploadBtn, backgroundColor:'rgba(255,255,255,0.05)' }}>
                          📂 Files
                        </button>
                      </div>
                    ...
                    <div style={{ display:'flex', alignItems:'center', gap:'24px' }}>
                      <label style={{ display:'flex', alignItems:'center', gap:'8px', cursor:'pointer', fontSize:'13px', color:'rgba(255,255,255,0.6)' }}>
                        <input type="checkbox" checked={itemForm.isSpecial} onChange={e => setItemForm({ ...itemForm, isSpecial:e.target.checked })} />
                        Mark as Special
                      </label>
                      <label style={{ display:'flex', alignItems:'center', gap:'8px', cursor:'pointer', fontSize:'13px', color:'rgba(255,255,255,0.6)' }}>
            ...
            <div style={{...S.card, marginBottom:'24px'}}>
              ...
              <div style={S.menuTableContainer}>
                <table style={S.menuTable}>
                  <thead>
                    <tr>
                      <th style={S.menuTh}>Image</th>
                      <th style={S.menuTh}>Category</th>
                      <th style={S.menuTh}>Name</th>
                      <th style={S.menuTh}>Price</th>
                      <th style={S.menuTh}>Special</th>
                      <th style={S.menuTh}>Available</th>
                      <th style={S.menuTh}>Actions</th>
                    </tr>
                  </thead>
                  <tbody>
                    {menuItems
                      .filter(item => 
                        (menuCategoryFilter === 'all' || item.categoryId == menuCategoryFilter) &&
                      ...
                      .map(item => {
                        ...
                        return (
                          <tr key={item.id} style={S.menuTr} onMouseEnter={(e) => e.target.closest('tr').style.backgroundColor = S.menuTrHover.backgroundColor} onMouseLeave={(e) => e.target.closest('tr').style.backgroundColor = 'transparent'}>
                            <td style={S.menuTd}>
                              {item.imageUrl ? (
                                <img src={item.imageUrl} alt={item.name} style={S.menuImage} />
                              ) : (
                                <div style={{ width:'40px', height:'40px', borderRadius:'6px', backgroundColor:'#1a1a1a', display:'flex', alignItems:'center', justifyContent:'center', fontSize:'16px' }}>🍽️</div>
                              )}
                            </td>
            ...
            {menuItems.length===0 && categories.length>0 && (
              <div style={S.empty}><p>No menu items yet — click "+ Menu Item" to add</p></div>
            )}
            {categories.length===0 && (
              <div style={S.empty}><p style={{ fontSize:'40px', marginBottom:'12px' }}>🍽️</p><p>Create a category first, then add menu items</p></div>
            )}
          </div>
        )}
        ...
        {activeTab==='reviews' && (
          <div>
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'24px' }}>
              ...
              <button onClick={() => setShowReviewForm(!showReviewForm)} style={S.btnPrimary(accent)}>
                ...
                <form onSubmit={saveReview}>
                  <div style={S.grid2}>
                    <div>
                      ...
                      <input value={reviewForm.customerName} onChange={e => setReviewForm({ ...reviewForm, customerName:e.target.value })} required style={S.input} />
                    </div>
                    <div>
                      <label style={S.label}>Source</label>
                      <input value={reviewForm.source} onChange={e => setReviewForm({ ...reviewForm, source:e.target.value })} placeholder="Google, Yelp..." style={S.input} />
                    </div>
                    <div>
                      <label style={S.label}>Rating</label>
                      <div style={{ display:'flex', gap:'6px' }}>
                        {[1,2,3,4,5].map(i => (
                          <button key={i} type="button" onClick={() => setReviewForm({ ...reviewForm, rating:i })} style={{ background:'none', border:'none', cursor:'pointer', fontSize:'28px', color: i<=reviewForm.rating ? '#FBBF24' : 'rgba(255,255,255,0.15)' }}>★</button>
                        ))}
                      </div>
                    </div>
                    <div style={{ gridColumn:'1/-1' }}>
                      <label style={S.label}>Review *</label>
                      <textarea value={reviewForm.content} onChange={e => setReviewForm({ ...reviewForm, content:e.target.value })} required rows={3} style={S.textarea} />
                    </div>
                  </div>
                  <div style={{ display:'flex', gap:'8px', marginTop:'16px' }}>
            ...
            {reviews.length===0 ? (
              <div style={S.empty}><p style={{ fontSize:'40px', marginBottom:'12px' }}>⭐</p><p>No reviews yet</p></div>
            ) : reviews.map(review => (
                ...
                <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start' }}>
                  <div style={{ flex:1 }}>
                    <div style={{ display:'flex', alignItems:'center', gap:'10px', marginBottom:'6px' }}>
                      ...
                      <span style={{ marginLeft:'auto', ...S.badge(review.isApproved?'#4ade80':'#fbbf24', review.isApproved?'rgba(74,222,128,0.15)':'rgba(251,191,36,0.15)') }}>
                        {review.isApproved ? 'Published' : 'Pending'}
                      </span>
                    </div>
                    <div style={{ display:'flex', gap:'2px', marginBottom:'8px' }}>
                      {[1,2,3,4,5].map(i => <span key={i} style={{ color: i<=review.rating ? '#FBBF24' : 'rgba(255,255,255,0.15)', fontSize:'14px' }}>★</span>)}
                    </div>
                    <p style={{ color:'rgba(255,255,255,0.6)', fontSize:'13px', lineHeight:1.6 }}>{review.content}</p>
                  </div>
                  <div style={{ display:'flex', gap:'8px', marginLeft:'16px' }}>
                    <button onClick={() => toggleApprove(review.id)} style={review.isApproved ? S.btnDanger : S.btnSuccess}>
          ...
          <div>
            ...
            <div style={{ ...S.card, marginBottom:'24px' }}>
              <div style={{ display:'flex', gap:'10px' }}>
                ...
                <button onClick={() => setFilePicker({ onSelect: (url) => addGalleryImage(url) })} style={{ ...S.uploadBtn, backgroundColor:'rgba(255,255,255,0.08)', color:'rgba(255,255,255,0.7)' }}>
                  📁 From Files
                </button>
                <label style={S.uploadBtn}>
                  📁 Upload Image
                  <input type="file" accept="image/*" style={{ display:'none' }} onChange={async e => {
                    const file = e.target.files[0]; if (!file) return;
                    try { const url = await uploadFile(file, slug, 'gallery'); await addGalleryImage(url); }
                    catch(err) { alert('Upload failed: '+err.message); }
                  }} />
                </label>
              </div>
            ...
            {gallery.length===0 ? (
              <div style={S.empty}><p style={{ fontSize:'40px', marginBottom:'12px' }}>🖼️</p><p>No gallery images yet</p></div>
            ) : (
              <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(180px, 1fr))', gap:'12px' }}>
                {gallery.map(img => (
                  <div key={img.id} style={{ position:'relative', borderRadius:'10px', overflow:'hidden', aspectRatio:'1' }}>
                    ...
                    <div style={{ position:'absolute', inset:0, backgroundColor:'rgba(0,0,0,0)', display:'flex', alignItems:'center', justifyContent:'center', gap:'6px', transition:'background 0.3s' }}
                      onMouseEnter={e => { e.currentTarget.style.backgroundColor='rgba(0,0,0,0.6)'; e.currentTarget.querySelectorAll('button').forEach(b => b.style.opacity='1'); }}
                      onMouseLeave={e => { e.currentTarget.style.backgroundColor='rgba(0,0,0,0)'; e.currentTarget.querySelectorAll('button').forEach(b => b.style.opacity='0'); }}>
        ...
        {activeTab==='why-choose-us' && (
          <div>
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'24px' }}>
              ...
              <button onClick={() => { setShowWhyChooseUsForm(!showWhyChooseUsForm); setEditingWhyChooseUs(null); setWhyChooseUsForm({ icon:'', title:'', description:'', sortOrder:0 }); }} style={S.btnPrimary(accent)}>
                {showWhyChooseUsForm ? 'Cancel' : '+ Add Item'}
              </button>
            </div>
                ...
                <form onSubmit={saveWhyChooseUs}>
                  <div style={S.grid2}>
                    <div>
                      ...
                      <input value={whyChooseUsForm.icon} onChange={e => setWhyChooseUsForm({ ...whyChooseUsForm, icon:e.target.value })} placeholder="🍽️ 🌱 👨‍🍳" style={S.input} />
                    </div>
                    <div>
                      <label style={S.label}>Sort Order</label>
                      <input type="number" value={whyChooseUsForm.sortOrder} onChange={e => setWhyChooseUsForm({ ...whyChooseUsForm, sortOrder:parseInt(e.target.value) || 0 })} style={S.input} />
                    </div>
                    <div style={{ gridColumn:'1/-1' }}>
                      <label style={S.label}>Title *</label>
                      <input value={whyChooseUsForm.title} onChange={e => setWhyChooseUsForm({ ...whyChooseUsForm, title:e.target.value })} required style={S.input} />
                    </div>
                    <div style={{ gridColumn:'1/-1' }}>
                      <label style={S.label}>Description *</label>
                      <textarea value={whyChooseUsForm.description} onChange={e => setWhyChooseUsForm({ ...whyChooseUsForm, description:e.target.value })} required rows={3} style={S.textarea} />
                    </div>
                  </div>
                  <div style={{ display:'flex', gap:'8px', marginTop:'16px' }}>
                    <button type="submit" style={S.btnPrimary(accent)}>Save Item</button>
            ...
            {whyChooseUs.length === 0 ? (
              <div style={S.empty}>
                ...
                <p>No "Why Choose Us" items yet — click "+ Add Item" to add</p>
              </div>
            ) : (
        ...
        {activeTab==='filemanager' && (
          <div>
            ...
            <div style={{ height:'calc(100vh - 160px)' }}>
              <FileManager restaurantSlug={slug} />
            </div>
          </div>
        )}
            ...
            {editingInfo ? (
                ...
                <div style={{ ...S.card, marginBottom:'16px' }}>
                  ...
                  <div style={S.grid2}>
                    {[
                      ...
                      { key:'aboutShort', label:'Short Description' },
                      { key:'reservationText', label:'Reservation Text' },
                      ...
                      <div key={key}>
                        ...
                        <input value={infoForm[key]||''} onChange={e => setInfoForm({ ...infoForm, [key]:e.target.value })} required={required} style={S.input} />
                      </div>
                    ))}
                    <div style={{ gridColumn:'1/-1' }}>
                      <label style={S.label}>About Text</label>
                      <textarea value={infoForm.aboutText||''} onChange={e => setInfoForm({ ...infoForm, aboutText:e.target.value })} rows={4} style={S.textarea} />
                    </div>
                  </div>
                </div>
                  ...
                  <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:'16px' }}>
                    {[{ key:'accentColor', label:'Accent Color' }, { key:'primaryColor', label:'Primary Color' }, { key:'bgColor', label:'Background Color' }].map(({ key, label }) => (
                      <div key={key}>
                        <label style={S.label}>{label}</label>
                        <div style={{ display:'flex', gap:'8px' }}>
                          <input type="color" value={infoForm[key]||'#000'} onChange={e => setInfoForm({ ...infoForm, [key]:e.target.value })} style={{ width:'44px', height:'44px', border:'1px solid rgba(255,255,255,0.1)', borderRadius:'8px', cursor:'pointer', backgroundColor:'transparent', padding:'2px' }} />
                          <input value={infoForm[key]||''} onChange={e => setInfoForm({ ...infoForm, [key]:e.target.value })} style={{ ...S.input, flex:1 }} />
                        </div>
                      </div>
                    ))}
                  </div>
                ...
                <div style={{ ...S.card, marginBottom:'16px' }}>
                  ...
                  <div style={S.grid2}>
                    <div>
                      ...
                      <div style={{ display:'flex', gap:'8px' }}>
                        <input value={infoForm.logoUrl||''} onChange={e => setInfoForm({ ...infoForm, logoUrl:e.target.value })} placeholder="URL..." style={{ ...S.input, flex:1 }} />
                        <label style={S.uploadBtn}>
                          📁
                          <input type="file" accept="image/*" style={{ display:'none' }} onChange={async e => {
                            const file = e.target.files[0]; if (!file) return;
                            try { const url = await uploadFile(file, slug, 'logos'); setInfoForm(f => ({ ...f, logoUrl:url })); }
                            catch(err) { alert('Upload failed: '+err.message); }
                          }} />
                        </label>
                      </div>
                    ...
                    <div>
                      <label style={S.label}>Hero Video URL or Upload</label>
                      <div style={{ display:'flex', gap:'8px' }}>
                        <input value={infoForm.videoUrl||''} onChange={e => setInfoForm({ ...infoForm, videoUrl:e.target.value })} placeholder="https://..." style={{ ...S.input, flex:1 }} />
                        <label style={S.uploadBtn}>
                          📁
                          <input type="file" accept="video/*" style={{ display:'none' }} onChange={async e => {
                            const file = e.target.files[0]; if (!file) return;
                            try { const url = await uploadFile(file, slug, 'videos'); setInfoForm(f => ({ ...f, videoUrl:url })); }
                            catch(err) { alert('Upload failed: '+err.message); }
                          }} />
                        </label>
                      </div>
                    ...
                    <div style={{ gridColumn:'1/-1' }}>
                      ...
                      {(() => {
                        ...
                        return (
                          <div>
                            {banners.map((b,i) => (
                              <div key={i} style={{ display:'flex', gap:'10px', alignItems:'center', backgroundColor:'#0d0d0d', border:'1px solid rgba(255,255,255,0.08)', borderRadius:'8px', padding:'10px 14px', marginBottom:'8px' }}>
                                <img src={b} alt="" style={{ height:'36px', width:'56px', objectFit:'cover', borderRadius:'4px' }} />
                                <span style={{ flex:1, fontSize:'12px', color:'rgba(255,255,255,0.3)', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>Banner {i+1}</span>
                                <button type="button" onClick={() => updateBanners(banners.filter((_,idx)=>idx!==i))} style={{ background:'none', border:'none', cursor:'pointer', color:'#f87171', fontSize:'16px', padding:'0 4px' }}>✕</button>
                              </div>
                            ))}
                            {banners.length < 5 && (
                              <div style={{ display:'flex', gap:'8px' }}>
                                <input type="text" id="info-banner-url" placeholder="Paste image URL..." style={{ ...S.input, flex:1 }} />
                                <button type="button" onClick={() => { const inp = document.getElementById('info-banner-url'); if (inp.value) { updateBanners([...banners, inp.value]); inp.value=''; } }} style={S.btnPrimary(accent)}>Add</button>
                                <label style={S.uploadBtn}>
                                  📁 Upload
                                  <input type="file" accept="image/*" style={{ display:'none' }} onChange={async e => {
                                    const file = e.target.files[0]; if (!file) return;
                                    try { const url = await uploadFile(file, slug, 'banners'); updateBanners([...banners, url]); }
                                    catch(err) { alert('Upload failed: '+err.message); }
                                  }} />
                                </label>
                              </div>
                ...
                <div style={{ ...S.card, marginBottom:'16px' }}>
                  ...
                  <div style={S.grid2}>
                    {[{ key:'facebookUrl', label:'Facebook URL' }, { key:'instagramUrl', label:'Instagram URL' }, { key:'tiktokUrl', label:'TikTok URL' }, { key:'mapEmbedUrl', label:'Google Maps Embed URL' }].map(({ key, label }) => (
                      <div key={key}>
                        <label style={S.label}>{label}</label>
                        <input value={infoForm[key]||''} onChange={e => setInfoForm({ ...infoForm, [key]:e.target.value })} style={S.input} />
                      </div>
                    ))}
                  </div>
                </div>
                  ...
                  <div style={S.grid2}>
                    {DAYS.map(day => (
                      <div key={day} style={{ display:'flex', alignItems:'center', gap:'12px' }}>
                        <span style={{ fontSize:'12px', color:'rgba(255,255,255,0.4)', textTransform:'capitalize', width:'80px', flexShrink:0 }}>{day}</span>
                        <input value={infoForm.openingHoursObj?.[day]||''} onChange={e => setInfoForm({ ...infoForm, openingHoursObj:{ ...infoForm.openingHoursObj, [day]:e.target.value } })} placeholder="11:00 AM - 10:00 PM" style={{ ...S.input, flex:1, fontSize:'12px' }} />
                      </div>
                    ))}
                  </div>
                </div>
              ...
              <div style={S.card}>
                {[{ label:'Name', value:restaurant?.name }, { label:'Tagline', value:restaurant?.tagline }, { label:'Address', value:restaurant?.address }, { label:'Phone', value:restaurant?.phone }, { label:'Email', value:restaurant?.email }].map(({ label, value }) => (
                  <div key={label} style={{ borderBottom:'1px solid rgba(255,255,255,0.05)', paddingBottom:'14px', marginBottom:'14px' }}>
                    <p style={{ ...S.label, marginBottom:'4px' }}>{label}</p>
                    <p style={{ color:'rgba(255,255,255,0.7)', fontSize:'14px' }}>{value||'Not set'}</p>
                  </div>
                ))}
              </div>
            )}
